<script>
	...
	// get county parameter from url
    const queryString = window.location.search;
	const urlParams = new URLSearchParams(queryString);

    if (urlParams.has('county')) {
		$activeCounty = urlParams.get('county');
	}
...
<div class="">
	...
	<div class="grid grid-cols-2 gap-x-8 mx-12">
		...
		{#if y > 128}
			<div transition:fade class="flex fixed bottom-4 left-4 p-8 cursor-pointer text-white" on:click={scrollTop}>
