...
<div class="relative">
    ...
    {#if $activePlace} <!-- if there's an active place, show it first -->
        <div class="p-6 sticky top-0 mt-8 z-10 bg-black text-white border-b-2" transition:slide>
        ...
        <div class="pt-4">
            {#each $results as place} <!-- loop through places and render list items -->
                <div on:mouseenter={() => mouseIn(place)} on:mouseleave={mouseOut} on:click={() => mouseClick(place)}
                    class="bg-black bg-opacity-50 text-white mb-2 p-6 flex justify-between border-b-2 border-gray-400 cursor-pointer hover:text-blue-500">
                    <div>
